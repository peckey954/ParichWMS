...
/* ------------------------------------------------------------------
   สินค้า — ส่วนสุดท้ายของสายการตั้งค่า และส่วนเดียวที่โตได้ไม่จำกัด
   (อัปโหลด CSV ทีเดียวเป็นร้อยรายการ) จึงต้องมีค้นหากับแบ่งหน้า

   รายการเดียวรวมสินค้าทั้งหมด ไม่แยกตามคลัง เพราะสินค้าไม่ได้สังกัดคลัง
   มันสังกัด "ประเภท" แล้วประเภทเป็นตัวบอกว่าเข้าคลังไหน ดรอปดาวน์ประเภท
   จึงอยู่ที่แถวของสินค้าตัวนั้นเลย เปลี่ยนแล้วบรรทัดคลังใต้ชื่อเปลี่ยนตามทันที
...
export default function ProductsPage() {
  ...
  const { pages, safe, slice } = paginate(matched, page, PAGE_SIZE);

  const namedCategories = setup.categories.filter((c) => c.label.trim() !== "");
  const namedGroups = setup.groups.filter((g) => g.label.trim() !== "");
  ...
  function downloadTemplate() {
    ...
    const sampleGroup = namedGroups[0]?.label ?? "";
    const esc = (v: string) => v.replace(/"/g, '""');
    const csv =
      "﻿" +
      [
        "ชื่อสินค้า,ประเภท,หมวด,หน่วย,บรรจุภัณฑ์",
        // ครอบชื่อประเภท/หมวดด้วยคำพูดไว้ เผื่อคนตั้งชื่อที่มีจุลภาคอยู่ข้างใน
        `ตัวอย่างสินค้า,"${esc(sample)}","${esc(sampleGroup)}",ตัน,50 Kg`,
      ].join("\r\n");
  ...
  async function handleFile(file: File) {
    ...
    setPreview(
      importProductsCsv(await file.text(), setup.categories, setup.groups),
    );
  }
  ...
  function confirmImport() {
    ...
    const n = addProducts(
      preview.rows.map((r) => ({
        ...r,
        enabled: true,
        warehouseId: warehouseForCategory(setup, r.categoryId),
      })),
    );
    // ของใหม่ต่อหัวรายการ ต้องกลับไปหน้าแรกและล้างคำค้น ไม่งั้นค้างอยู่หน้าที่
    // มองไม่เห็นของที่เพิ่งเข้ามา แล้วดูเหมือนอัปโหลดไม่ติด
    setPage(1);
    setQuery("");
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pt-3 pb-24 sm:px-6 sm:pt-5">
        ...
        <div className="mt-2 flex flex-wrap items-start justify-between gap-3 sm:mt-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              สินค้าทั้งหมดที่ขอซื้อได้ — เลือกประเภทและหมวดที่ท้ายแต่ละรายการ
              ประเภทเป็นตัวบอกว่าของเข้าคลังไหน ส่วนหมวดใช้จัดกลุ่มอย่างเดียว
            </p>
          </div>
        ...
        {uncategorized > 0 && (
          <p className="mt-3 rounded-xl border border-chip-yellow-foreground/40 bg-chip-yellow px-4 py-3 text-sm">
            มี {uncategorized} รายการที่ยังไม่ระบุประเภท —
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <InputGroup className="min-w-0 flex-1 bg-card">
            ...
            <InputGroupInput
              ...
              onChange={(e) => {
                ...
                // คำค้นเปลี่ยนแล้วจำนวนหน้าเปลี่ยนตาม ถ้าไม่รีเซ็ตจะค้างอยู่หน้าที่ว่างเปล่า
                setPage(1);
              }}
            />
          </InputGroup>
          <p className="shrink-0 text-sm text-muted-foreground">
            {q === ""
              ...
              : `${matched.length} จาก ${products.length} รายการ`}
          </p>
        </div>
          ...
          <div className="mt-3 overflow-hidden rounded-xl border border-border bg-card">
            {slice.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="grid gap-2 border-t border-border px-3 py-3 first:border-0 @3xl:grid-cols-[1fr_13rem_11rem_auto] @3xl:items-center @3xl:gap-3"
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 truncate text-sm text-muted-foreground">
                      ...
                      {" · "}
                      {p.categoryId === "" ? (
                        <span className="text-danger-strong">ยังไม่ระบุประเภท</span>
                      ) : dest.length > 0 ? (
                        `เข้า ${dest
                          .map((w) => w.label || "(ยังไม่ได้ตั้งชื่อ)")
                          .join(", ")}`
                      ) : (
                        /* มีประเภทแต่ประเภทนั้นยังไม่ได้เลือกคลัง — คนละสาเหตุกับข้างบน
                           แยกข้อความไว้ จะได้รู้ว่าต้องไปแก้ที่หน้าไหน */
                        <span className="text-danger-strong">
                          ประเภทนี้ยังไม่ได้เลือกคลัง
                        </span>
                      )}
                    </p>
                  </div>
                  ...
                  {namedCategories.length === 0 ? (
                    ...
                    >
                      ยังไม่มีประเภท — ไปเพิ่มประเภทก่อน
                    </Link>
                  ) : (
                    <Select
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={NO_CATEGORY}>ยังไม่ระบุประเภท</SelectItem>
                  ...
                  {/* หมวดไม่กระทบเส้นทางเข้าคลัง ไม่ระบุก็ไม่เป็นไร จึงไม่มีคำเตือนแดง
                      เหมือนประเภท และไม่ต้องมีลิงก์บังคับให้ไปสร้างหมวดก่อน */}
                  {namedGroups.length === 0 ? (
                    <Link
                      href="/pr/setup/groups"
                      className="text-sm text-primary underline underline-offset-2"
                    >
                      ยังไม่มีหมวด — ไปเพิ่มหมวด
                    </Link>
                  ) : (
                    <Select
                      value={p.groupId || NO_GROUP}
                      onValueChange={(v) =>
                        setProductGroup(p.id, v === NO_GROUP ? "" : v)
                      }
                    >
                      <SelectTrigger
                        aria-label={`หมวดของ ${p.name}`}
                        className="w-full bg-card"
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value={NO_GROUP}>ยังไม่ระบุหมวด</SelectItem>
                        {namedGroups.map((g) => (
      ...
      <SetupFooter
        ...
        onSave={() =>
          toast.success("บันทึกสินค้าแล้ว", {
            description: `${products.length} รายการ${
              uncategorized > 0 ? ` · ${uncategorized} รายการยังไม่ระบุประเภท` : ""
            }`,
          })
        }
      />
      ...
      <AddProductDialog
        ...
        onAdd={(p) => {
          addProduct({
            ...p,
            enabled: true,
            warehouseId: warehouseForCategory(setup, p.categoryId),
          });
          setPage(1);
          setQuery("");
        ...
        <div className="grid gap-4 @lg:grid-cols-2">
          ...
          <div className="grid gap-1.5">
            ...
            <Select value={groupId} onValueChange={setGroupId}>
              <SelectTrigger id="p-grp" className="w-full bg-card">
